...
const InputFormD = () => {
  const initialState = {};
  const [FormDataD, setFormDataD] = useState(initialState);
  ...
  const handleChange = (e) =>
    setFormDataD({ ...FormDataD, [e.target.name]: e.target.value });

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!loggedInUser) return alert("Logged-in user not detected!");
    if (alreadySubmitted) {
      alert("You have already submitted today's report!");
      return;
    }
    ...
    try {
      await axios.post("https://monitor-r0u9.onrender.com/api/form-datad", {
        ...FormDataD,
        userName: loggedInUser,
      });
      alert("Form submitted successfully!");
      setFormDataD(initialState);
      fetchReports();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-4 w-full max-w-5xl mx-auto border rounded shadow mt-4">
      <h2 className="text-xl font-bold mb-4">Input Form for SBU-D</h2>
      ...
      {loggedInUser && (
        <UserReportD loggedInUser={loggedInUser} allReports={allReports} />
      )}
    </div>
  );
};
